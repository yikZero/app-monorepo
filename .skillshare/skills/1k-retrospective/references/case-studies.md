...
## Case: Desktop browser address bar and refresh dead after idle
**Date**: 2026-09-21 | **Platforms**: Desktop (macOS reported; Win/Linux share the custom title-bar synthesizer)
**Symptom**: After leaving the desktop app in the background for about an hour, Discovery address-bar and refresh clicks did nothing. Cmd +/- zoom restored clicks.
**Root Cause**: Synthesized title-bar overlays used a full-header `drag` region plus `no-drag` holes. After idle, Electron/Chromium kept the drag cache and dropped the holes, so header controls were covered. Resume/focus did not rebuild overlays; only resize (zoom) did.
**Fix**: Emit drag overlays only for the empty remainder of each zone (zone minus control rects), keep no-drag holes as a safety net, and immediately recompute overlays when the window becomes active/visible/focused or the page resumes.
**Catchable by**: Section 3: platform-specific Electron title-bar hit testing; NEW — a `-webkit-app-region: drag` overlay must not cover controls, and native region caches must be rebuilt on foreground resume, not only on resize
