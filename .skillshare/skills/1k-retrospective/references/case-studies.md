...
## Case: Prime gift offer popped in and shifted the page
**Date**: 2026-09-24 | **Platforms**: iOS, Android, Desktop, Web, Extension
**Symptom**: OK-64039. When the Prime gift offer appeared, the surrounding content jumped. On wallet setup the centered orb, title, and enter button shifted; on device details the sections below were pushed down.
**Root Cause**: `PrimeGiftOffer` rendered nothing until eligibility arrived, then inserted a full-height card in one frame. Parent `gap` would also count a 0-height child, so a collapsed placeholder could not sit in that column without adding space.
**Fix**: Grow the offer from height 0 with `HeightTransition` when it appears after the page is already visible. Keep a cached device-details offer at its final height. Put the offer's top spacing inside the measured child and group it with the previous block so a hidden offer adds no gap.
**Catchable by**: Section 3: UI changes on the narrowest and widest layouts; Section 4: async data arrival must not insert layout without a transition when the page is already showing
