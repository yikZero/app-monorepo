...
function ControlChip({
  ...
  return (
    <Button
      testID={`pro2-prime-control-${label}`}
      size="small"
...
function ControlRow({
  ...
  return (
    <XStack
      testID={`pro2-prime-condition-${label}`}
      alignItems="flex-start"
      gap="$2"
      flexWrap="wrap"
    >
...
function Pro2PrimeGiftDemo() {
  ...
  const openLogin = useCallback(() => {
    showDialog(
      Dialog.show({
        testID: 'pro2-prime-dialog-login',
        title: '登录 OneKey ID',
  ...
  const openNotify = useCallback(() => {
    showDialog(
      Dialog.show({
        testID: 'pro2-prime-dialog-notify',
        icon: 'BellOutline',
        title: '开启通知',
        description:
        ...
        onConfirm: async ({ close }) => {
          ...
          showDialog(
            Dialog.show({
              testID: 'pro2-prime-dialog-notify-permission',
              title: '允许通知',
  ...
  const openBenefits = useCallback(() => {
    showDialog(
      Dialog.show({
        testID: 'pro2-prime-dialog-benefits',
        title: 'Prime 会员权益',
        renderContent: (
  ...
  const onClaimPrimary = useCallback(() => {
    ...
    if (!current.verified) {
      showDialog(
        Dialog.show({
          testID: 'pro2-prime-dialog-verify',
          title: '验证设备',
  ...
  const openInvite = () => {
    ...
    showDialog(
      Dialog.show({
        testID: 'pro2-prime-dialog-invite',
        title: '填写邀请码',
                ...
                >
                  ...
                  >
                    <XStack alignItems="center" gap="$2" mb="$1">
                      <Icon name={icon} size="$6" />
                      <SizableText size="$headingMd">6 个月 Prime</SizableText>
                    </XStack>
                    <SizableText size="$bodyMd" color="$textSubdued">
                      领取账号：{ACCOUNT}
                    </SizableText>
